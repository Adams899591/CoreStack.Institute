...
} on this my code i hav added             $table->id();
            $table->foreignId('user_id')->constrained()->onDelete('cascade');
            $table->foreignId('course_id')->constrained();
            $table->foreignId('academic_period_id')->constrained();
            $table->enum('status', ['registered', 'dropped'])->default('registered'); student_course_registrations table to it and i will like u to add the facker crecor to it with out changing my code or breaking the existing logic keep tha academy period id to be 12 pass in the user id and course id on the year  2025/2026 and where the semester is second so it summery we are making the student to have a register courses of the last section